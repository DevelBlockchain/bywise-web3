...
class Wallet {
    constructor(config) {
        ...
        this.getAddress = (tag = '') => {
            return BywiseHelper_1.BywiseHelper.encodeBWSAddress(false, this.account.address, tag);
        };
        this.getStealthAddress = (account, index, tag = '') => {
            ...
            return BywiseHelper_1.BywiseHelper.encodeBWSAddress(false, node.address, tag);
        };
