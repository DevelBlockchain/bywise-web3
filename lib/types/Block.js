...
class Block {
    ...
    isValid() {
        ...
        if (!BywiseHelper_1.BywiseHelper.isStringArray(this.slices))
        ...
        if (this.transactionsCount > this.slices.length)
            throw new Error('invalid block transactionsCount');
        ...
        if (!BywiseHelper_1.BywiseHelper.isStringArray(this.externalTxID))
            throw new Error('invalid array');
        for (let i = 0; i < this.externalTxID.length; i++) {
            let txId = this.externalTxID[i];
            if (!BywiseHelper_1.BywiseHelper.isValidHash(txId))
